...
var ctilocations = L.geoJson(null, {
    ...
    if (feature.properties) {
      //var content = "<table class='table table-striped table-bordered table-condensed'>" + "<tr><th>Name</th><td>" + feature.properties.NAME + "</td></tr>" + "<tr><th>Phone</th><td>" + feature.properties.TEL + "</td></tr>" + "<tr><th>Address</th><td>" + feature.properties.ADRESS1 + "</td></tr>" + "<tr><th>Website</th><td><a class='url-break' href='" + feature.properties.URL + "' target='_blank'>" + feature.properties.URL + "</a></td></tr>" + "<table>";
			var content = "<div><h4>" + feature.properties.title +"</h4>" +
      ...
      layer.on({
        click: function (e) {
          ...
          //$("#featureModal").modal("show");
					...
					$('#sidebarMap').show();
					//sidebarMonitor.hide();
          $("#map-project-info-details").html(content);
      ...
      //$("#museum-table tbody").append('<tr style="cursor: pointer;" onclick="sidebarClick('+L.stamp(layer)+'); return false;"><td class="museum-name">'+layer.feature.properties.NAME+'<i class="fa fa-chevron-right pull-right"></td></tr>');
			...
			'<td class="container">' +
			'<div class="col-xs-10"><span class="project-title" rel="tooltip" id="project-title-'+L.stamp(layer)+'" data-toggle="tooltip" title="'+layer.feature.properties.title+'">'+layer.feature.properties.title+'</span></div>' +
...
$('#opener').on('click', function() {       
    var myPanel = $('#monitor-panel');
    if (myPanel.hasClass("visible")) {
        myPanel.removeClass('visible').animate({'margin-left':'-300px'});
    } else {
        myPanel.addClass('visible').animate({'margin-left':'0px'});
    }   
    return false;   
});