...
(function (factory) {
	...
	(function (L) {
		...
		function (a) {
			...
			function d(a) {
				return c(a[0], a[a.length - 1]) || a.push(a[0]),
				a
			}
			function e(a) {
				var b,
			...
			a.Util.arcgisToGeojson = function (c, d) {
				var e = {};
				return "number" == typeof c.x && "number" == typeof c.y && (e.type = "Point", e.coordinates = [c.x, c.y]),
				c.points && (e.type = "MultiPoint", e.coordinates = c.points.slice(0)),
				c.paths && (1 === c.paths.length ? (e.type = "LineString", e.coordinates = c.paths[0].slice(0)) : (e.type = "MultiLineString", e.coordinates = c.paths.slice(0))),
				c.rings && (e = j(c.rings.slice(0))),
				(c.geometry || c.attributes) && (e.type = "Feature", e.geometry = c.geometry ? a.Util.arcgisToGeojson(c.geometry) : null, e.properties = c.attributes ? b(c.attributes) : null, c.attributes && (e.id = c.attributes[d] || c.attributes.OBJECTID || c.attributes.FID)),
				e
			},
			a.Util.geojsonToArcGIS = function (c, d) {
				...
				case "Point":
					g.x = c.coordinates[0],
					g.y = c.coordinates[1],
					g.spatialReference = f;
					break;
				case "MultiPoint":
					g.points = c.coordinates.slice(0),
					g.spatialReference = f;
					break;
				case "LineString":
					g.paths = [c.coordinates.slice(0)],
					g.spatialReference = f;
					break;
				case "MultiLineString":
					...
					break;
				case "Polygon":
					g.rings = k(c.coordinates.slice(0)),
					g.spatialReference = f;
					break;
				case "MultiPolygon":
					g.rings = l(c.coordinates.slice(0)),
					g.spatialReference = f;
					break;
				case "Feature":
					c.geometry && (g.geometry = a.Util.geojsonToArcGIS(c.geometry, d)),
					g.attributes = c.properties ? b(c.properties) : {},
					c.id && (g.attributes[d] = c.id);
					break;
				case "FeatureCollection":
					for (g = [], e = 0; e < c.features.length; e++)
						g.push(a.Util.geojsonToArcGIS(c.features[e], d));
					break;
				case "GeometryCollection":
					for (g = [], e = 0; e < c.geometries.length; e++)
						g.push(a.Util.geojsonToArcGIS(c.geometries[e], d))
				}
				return g
			},
			a.Util.responseToFeatureCollection = function (b, c) {
				...
				if (c)
					d = c;
				else if (b.objectIdFieldName)
					d = b.objectIdFieldName;
				else if (b.fields) {
				...
				if (g.length)
					for (var h = g.length - 1; h >= 0; h--)
						f.features.push(a.Util.arcgisToGeojson(g[h], d));
				return f
			},
		...
		function (a) {
			function b(a) {
				...
				for (var c in a)
					if (a.hasOwnProperty(c)) {
						...
						b.length && (b += "&"),
						d = "[object Array]" === f || "[object Object]" === f ? JSON.stringify(e) : "[object Date]" === f ? e.valueOf() : e,
			...
			a.Request = {
				request : function (a, d, e, f) {
					var g = b(d),
					h = c(e, f),
					i = (a + "?" + g).length;
					if (2e3 >= i && L.esri.Support.CORS)
						h.open("GET", a + "?" + g), h.send(null);
					else if (i > 2e3 && L.esri.Support.CORS)
						h.open("POST", a), h.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"), h.send(g);
					else {
						if (2e3 >= i && !L.esri.Support.CORS)
							return L.esri.Request.get.JSONP(a, d, e, f);
						if (console && console.warn)
							return void console.warn("a request to " + a + " was longer then 2000 characters and this browser cannot make a cross-domain post request. Please use a proxy http://esri.github.io/esri-leaflet/api-reference/request.html")
					}
					return h
				},
				post : {
				...
				get : {
					...
					JSONP : function (a, c, e, f) {
						...
						h.src = a + "?" + b(c),
						h.id = g,
						window._EsriLeafletCallbacks[g] = function (a) {
							if (window._EsriLeafletCallbacks[g] !== !0) {
								...
								e.call(f, b, a),
								window._EsriLeafletCallbacks[g] = !0
							}
						},
						d++, {
							id : g,
			...
			a.get = a.Support.CORS ? a.Request.get.CORS : a.Request.get.JSONP,
			a.post = a.Request.post.XMLHTTP,
			a.request = a.Request.request
		}
		(EsriLeaflet),
		EsriLeaflet.Services.Service = L.Class.extend({
				...
				initialize : function (a, b) {
					this.url = EsriLeaflet.Util.cleanUrl(a),
					this._requestQueue = [],
				...
				_request : function (a, b, c, d, e) {
					...
					if (this.options.token && (c.token = this.options.token), this._authenticating)
						return void this._requestQueue.push([a, b, c, d, e]);
					var g = this.options.proxy ? this.options.proxy + "?" + this.url + b : this.url + b;
				...
				_createServiceCallback : function (a, b, c, d, e) {
					...
					return L.Util.bind(function (g, h) {
						!g || 499 !== g.code && 498 !== g.code ? (d.call(e, g, h), g ? this.fire("requesterror", {
								url : this.url + b,
								params : c,
								message : g.message,
								code : g.code,
								...
								params : c,
								method : a
							})) : (this._authenticating = !0, this._requestQueue.push(f), this.fire("authenticationrequired", {
		...
		EsriLeaflet.Services.service = function (a, b) {
			return new EsriLeaflet.Services.Service(a, b)
		},
		EsriLeaflet.Services.FeatureLayer = EsriLeaflet.Services.Service.extend({
				...
				addFeature : function (a, b, c) {
					return delete a.id,
					a = EsriLeaflet.Util.geojsonToArcGIS(a),
					this.post("addFeatures", {
						features : [a]
					}, function (a, c) {
						var d = c && c.addResults ? c.addResults[0] : void 0;
						b && b.call(this, a || c.addResults[0].error, d)
					}, c)
				},
				updateFeature : function (a, b, c) {
					return a = EsriLeaflet.Util.geojsonToArcGIS(a, this.options.idAttribute),
		...
		EsriLeaflet.Services.featureLayer = function (a, b) {
			return new EsriLeaflet.Services.FeatureLayer(a, b)
		},
		...
		EsriLeaflet.Services.mapService = function (a, b) {
			return new EsriLeaflet.Services.MapService(a, b)
		},
		...
		EsriLeaflet.Services.imageService = function (a, b) {
			return new EsriLeaflet.Services.ImageService(a, b)
		},
		EsriLeaflet.Tasks.Task = L.Class.extend({
				...
				generateSetter : function (a, b) {
					...
					return a = c ? c[1] : a,
					c ? L.Util.bind(function (b) {
						return this.params[a] = L.Util.isArray(b) ? b.join(",") : b,
						this
					}, b) : L.Util.bind(function (b) {
						return this.params[a] = b,
						this
					}, b)
				},
				initialize : function (a, b) {
					if (a.url && a.request ? (this._service = a, this.url = a.url) : this.url = EsriLeaflet.Util.cleanUrl(a), this.params = L.Util.extend({}, this.params || {}), this.setters)
				...
				token : function (a) {
					return this._service ? this._service.authenticate(a) : this.params.token = a,
					this
				},
				...
				_request : function (a, b, c, d, e) {
					var f = this.options.proxy ? this.options.proxy + "?" + this.url + b : this.url + b;
		...
		EsriLeaflet.Tasks.Query = EsriLeaflet.Tasks.Task.extend({
				...
				nearby : function (a, b) {
					return a = L.latLng(a),
					this.params.geometry = [a.lng, a.lat].join(","),
					this.params.geometryType = "esriGeometryPoint",
					this.params.spatialRel = "esriSpatialRelIntersects",
					this.params.units = "esriSRUnit_Meter",
					this.params.distance = b,
					this.params.inSr = 4326,
					this
				},
				where : function (a) {
					return this.params.where = a.replace(/"/g, "'"),
					this
				},
				between : function (a, b) {
					return this.params.time = [a.valueOf(), b.valueOf()].join(),
					this
				},
				fields : function (a) {
					return this.params.outFields = L.Util.isArray(a) ? a.join(",") : a,
					this
				},
				simplify : function (a, b) {
					var c = Math.abs(a.getBounds().getWest() - a.getBounds().getEast());
					return this.params.maxAllowableOffset = c / a.getSize().y * b,
					this
				},
				orderBy : function (a, b) {
					return b = b || "ASC",
					this.params.orderByFields = this.params.orderByFields ? this.params.orderByFields + "," : "",
					this.params.orderByFields += [a, b].join(" "),
					this
				},
				returnGeometry : function (a) {
					return this.params.returnGeometry = a,
					this
				},
				run : function (a, b) {
					return this._cleanParams(),
					this.request(function (c, d) {
				...
				pixelSize : function (a) {
					return a = L.point(a),
					this.params.pixelSize = [a.x, a.y].join(","),
					this
				},
				layer : function (a) {
					return this.path = a + "/query",
					this
				},
				...
				_setGeometry : function (a) {
					return this.params.inSr = 4326,
					a instanceof L.LatLngBounds ? (this.params.geometry = EsriLeaflet.Util.boundsToExtent(a), void(this.params.geometryType = "esriGeometryEnvelope")) : (a.getLatLng && (a = a.getLatLng()), a instanceof L.LatLng && (a = {
								type : "Point",
								coordinates : [a.lng, a.lat]
							}), a instanceof L.GeoJSON && (a = a.getLayers()[0].feature.geometry, this.params.geometry = EsriLeaflet.Util.geojsonToArcGIS(a), this.params.geometryType = EsriLeaflet.Util.geojsonTypeToArcGIS(a.type)), a.toGeoJSON && (a = a.toGeoJSON()), "Feature" === a.type && (a = a.geometry), "Point" === a.type || "LineString" === a.type || "Polygon" === a.type ? (this.params.geometry = EsriLeaflet.Util.geojsonToArcGIS(a), void(this.params.geometryType = EsriLeaflet.Util.geojsonTypeToArcGIS(a.type))) : void(console && console.warn && console.warn("invalid geometry passed to spatial query. Should be an L.LatLng, L.LatLngBounds or L.Marker or a GeoJSON Point Line or Polygon object")))
				}
			}),
		EsriLeaflet.Tasks.query = function (a, b) {
			return new EsriLeaflet.Tasks.Query(a, b)
		},
		EsriLeaflet.Tasks.Find = EsriLeaflet.Tasks.Task.extend({
				...
				layerDefs : function (a, b) {
					...
					this.params.layerDefs += [a, b].join(":"),
					this
				},
				simplify : function (a, b) {
					var c = Math.abs(a.getBounds().getWest() - a.getBounds().getEast());
		...
		EsriLeaflet.Tasks.find = function (a, b) {
			return new EsriLeaflet.Tasks.Find(a, b)
		},
		...
		EsriLeaflet.Tasks.IdentifyImage = EsriLeaflet.Tasks.Identify.extend({
				...
				setPixelSize : function (a) {
					return this.params.pixelSize = a.join ? a.join(",") : a,
					this
				},
				...
				_responseToGeoJSON : function (a) {
					...
					if (a.properties && a.properties.Values && (e.pixel.properties.values = a.properties.Values), c && c.features && (e.catalogItems = EsriLeaflet.Util.responseToFeatureCollection(c), d && d.length === e.catalogItems.features.length))
						for (var f = d.length - 1; f >= 0; f--)
							e.catalogItems.features[f].properties.catalogItemVisibility = d[f];
					return e
				}
			}),
		EsriLeaflet.Tasks.identifyImage = function (a, b) {
			return new EsriLeaflet.Tasks.IdentifyImage(a, b)
		},
		EsriLeaflet.Tasks.IdentifyFeatures = EsriLeaflet.Tasks.Identify.extend({
				...
				on : function (a) {
					...
					c = a.getSize();
					return this.params.imageDisplay = [c.x, c.y, 96].join(","),
					this.params.mapExtent = [b.xmin, b.ymin, b.xmax, b.ymax].join(","),
					this
				},
				at : function (a) {
					return a = L.latLng(a),
				...
				simplify : function (a, b) {
					var c = Math.abs(a.getBounds().getWest() - a.getBounds().getEast());
					return this.params.maxAllowableOffset = c / a.getSize().y * (1 - b),
					this
				},
				run : function (a, b) {
					return this.request(function (c, d) {
		...
		EsriLeaflet.Tasks.identifyFeatures = function (a, b) {
			return new EsriLeaflet.Tasks.IdentifyFeatures(a, b)
		},
		function (a) {
			...
			a.Layers.BasemapLayer = L.TileLayer.extend({
					statics : {
						TILES : {
							...
							DarkGray : {
								urlTemplate : b + "//tiles{s}.arcgis.com/tiles/P3ePLMYs2RVChkJx/arcgis/rest/services/World_Dark_Gray_Base_Beta/MapServer/tile/{z}/{y}/{x}",
								options : {
									hideLogo : !1,
									logoPosition : "bottomright",
									minZoom : 1,
							...
							DarkGrayLabels : {
								urlTemplate : b + "//tiles{s}.arcgis.com/tiles/P3ePLMYs2RVChkJx/arcgis/rest/services/World_Dark_Gray_Reference_Beta/MapServer/tile/{z}/{y}/{x}",
								options : {
									hideLogo : !0,
									logoPosition : "bottomright",
									minZoom : 1,
					...
					initialize : function (b, c) {
						var d;
						if ("object" == typeof b && b.urlTemplate && b.options)
							d = b;
						else {
							if ("string" != typeof b || !a.BasemapLayer.TILES[b])
						...
						L.TileLayer.prototype.initialize.call(this, d.urlTemplate, L.Util.setOptions(this, e)),
						d.attributionUrl && this._getAttributionData(d.attributionUrl)
					},
					onAdd : function (b) {
						this.options.hideLogo || (this._logo = new a.Controls.Logo({
									position : this.options.logoPosition
								}).addTo(b)),
						L.TileLayer.prototype.onAdd.call(this, b),
						b.on("moveend", this._updateMapAttribution, this)
					},
					onRemove : function (a) {
						this._logo && a.removeControl(this._logo),
					...
					_updateMapAttribution : function () {
						if (this._map && this._map.attributionControl && this._attributions) {
							...
							g.innerHTML = a,
							g.style.maxWidth = .65 * this._map.getSize().x + "px",
			...
			a.BasemapLayer = a.Layers.BasemapLayer,
			a.Layers.basemapLayer = function (b, c) {
		...
		EsriLeaflet.Layers.RasterLayer = L.Class.extend({
				...
				onAdd : function (a) {
					if (this._map = a, this._update = L.Util.limitExecByInterval(this._update, this.options.updateInterval, this), a.options.crs && a.options.crs.code) {
						...
						this.options.bboxSR = b,
						this.options.imageSR = b
					}
					a.on("moveend", this._update, this),
					this._currentImage && this._currentImage._bounds.equals(this._map.getBounds()) ? a.addLayer(this._currentImage) : this._currentImage && (this._map.removeLayer(this._currentImage), this._currentImage = null),
					this._update(),
				...
				unbindPopup : function () {
					return this._map && (this._map.closePopup(this._popup), this._map.off("click", this._getPopupData, this), this._map.off("dblclick", this._resetPopupState, this)),
					this._popup = !1,
					this
				},
				onRemove : function () {
				...
				setTimeRange : function (a, b) {
					return this.options.from = a,
					this.options.to = b,
					this._update(),
					this
				},
				metadata : function (a, b) {
					return this._service.metadata(a, b),
					this
				},
				authenticate : function (a) {
					return this._service.authenticate(a),
					this
				},
				_renderImage : function (a, b) {
					if (this._map) {
						...
						c.once("load", function (a) {
							...
							c._bounds.equals(b) ? (this._currentImage = c, "front" === this.options.position ? this.bringToFront() : this.bringToBack(), this._map && this._currentImage._map ? this._currentImage.setOpacity(this.options.opacity) : this._currentImage._map.removeLayer(this._currentImage), d && this._map && this._map.removeLayer(d), d && d._map && d._map.removeLayer(d)) : this._map.removeLayer(c),
							this.fire("load", {
								bounds : b
							})
						}, this),
		...
		EsriLeaflet.Layers.DynamicMapLayer = EsriLeaflet.Layers.RasterLayer.extend({
				...
				initialize : function (a, b) {
					this.url = EsriLeaflet.Util.cleanUrl(a),
					this._service = new EsriLeaflet.Services.MapService(this.url, b),
				...
				setLayerDefs : function (a) {
					return this.options.layerDefs = a,
					this._update(),
					this
				},
				...
				setTimeOptions : function (a) {
					return this.options.timeOptions = a,
					this._update(),
					this
				},
				query : function () {
				...
				_getPopupData : function (a) {
					...
					c.layers(this.options.layers ? "visible:" + this.options.layers.join(",") : "visible"),
					c.run(b),
				...
				_buildExportParams : function () {
					...
					return this.options.layers && (e.layers = "show:" + this.options.layers.join(",")),
					this.options.layerDefs && (e.layerDefs = JSON.stringify(this.options.layerDefs)),
				...
				_requestExport : function (a, b) {
					"json" === this.options.f ? this._service.get("export", a, function (a, c) {
						this._renderImage(c.href, b)
					}, this) : (a.f = "image", this._renderImage(this.url + "export" + L.Util.getParamString(a), b))
				}
			}),
		EsriLeaflet.DynamicMapLayer = EsriLeaflet.Layers.DynamicMapLayer,
		EsriLeaflet.Layers.dynamicMapLayer = function (a, b) {
			return new EsriLeaflet.Layers.DynamicMapLayer(a, b)
		},
		EsriLeaflet.dynamicMapLayer = function (a, b) {
			return new EsriLeaflet.Layers.DynamicMapLayer(a, b)
		},
		EsriLeaflet.Layers.ImageMapLayer = EsriLeaflet.Layers.RasterLayer.extend({
				...
				initialize : function (a, b) {
					this.url = EsriLeaflet.Util.cleanUrl(a),
					this._service = new EsriLeaflet.Services.ImageService(this.url, b),
					this._service.on("authenticationrequired requeststart requestend requesterror requestsuccess", this._propagateEvent, this),
					L.Util.setOptions(this, b)
				},
				setPixelType : function (a) {
					return this.options.pixelType = a,
					this._update(),
					this
				},
				...
				setBandIds : function (a) {
					return this.options.bandIds = L.Util.isArray(a) ? a.join(",") : a.toString(),
					this._update(),
					this
				},
				getBandIds : function () {
				...
				setNoData : function (a, b) {
					return this.options.noData = L.Util.isArray(a) ? a.join(",") : a.toString(),
					b && (this.options.noDataInterpretation = b),
					this._update(),
					this
				},
				getNoData : function () {
				...
				_getPopupData : function (a) {
					...
					this.options.mosaicRule && c.setMosaicRule(this.options.mosaicRule),
					c.run(b),
					this._shouldRenderPopup = !0,
					this._lastClick = a.latlng
				},
				_buildExportParams : function () {
					...
					this.options.noData && (e.noData = this.options.noData),
				...
				_requestExport : function (a, b) {
					"json" === this.options.f ? this._service.get("exportImage", a, function (a, c) {
						this._renderImage(c.href, b)
					}, this) : (a.f = "image", this._renderImage(this.url + "exportImage" + L.Util.getParamString(a), b))
				}
			}),
		EsriLeaflet.ImageMapLayer = EsriLeaflet.Layers.ImageMapLayer,
		EsriLeaflet.Layers.imageMapLayer = function (a, b) {
			return new EsriLeaflet.Layers.ImageMapLayer(a, b)
		},
		EsriLeaflet.imageMapLayer = function (a, b) {
			return new EsriLeaflet.Layers.ImageMapLayer(a, b)
		},
		EsriLeaflet.Layers.TiledMapLayer = L.TileLayer.extend({
				initialize : function (a, b) {
					...
					this._service.on("authenticationrequired requeststart requestend requesterror requestsuccess", this._propagateEvent, this),
					this.tileUrl.match("://tiles.arcgisonline.com") && (this.tileUrl = this.tileUrl.replace("://tiles.arcgisonline.com", "://tiles{s}.arcgisonline.com"), b.subdomains = ["1", "2", "3", "4"]),
				...
				authenticate : function (a) {
					var b = "?token=" + a;
					return this.tileUrl = this.options.token ? this.tileUrl.replace(/\?token=(.+)/g, b) : this.tileUrl + b,
					this.options.token = a,
					this._service.authenticate(a),
					this
				},
				_propagateEvent : function (a) {
					a = L.extend({
		...
		EsriLeaflet.Layers.FeatureGrid = L.Class.extend({
				...
				_resetWrap : function () {
					...
					if (!b.infinite) {
						...
						b.wrapLng && (this._wrapLng = [Math.floor(a.project([0, b.wrapLng[0]]).x / c), Math.ceil(a.project([0, b.wrapLng[1]]).x / c)]),
						b.wrapLat && (this._wrapLat = [Math.floor(a.project([b.wrapLat[0], 0]).y / c), Math.ceil(a.project([b.wrapLat[1], 0]).y / c)])
					}
				},
				...
				_update : function () {
					if (this._map) {
						...
						if (!(b > this.options.maxZoom || b < this.options.minZoom)) {
							var d = L.bounds(a.min.divideBy(c).floor(), a.max.divideBy(c).floor());
							this._addCells(d),
							this._removeOtherCells(d)
						}
					}
				},
		...
		function (a) {
			...
			a.Layers.FeatureManager = a.Layers.FeatureGrid.extend({
					...
					initialize : function (c, d) {
						if (a.Layers.FeatureGrid.prototype.initialize.call(this, d), d = L.setOptions(this, d), this.url = a.Util.cleanUrl(c), this._service = new a.Services.FeatureLayer(this.url, d), "*" !== this.options.fields[0]) {
							for (var e = !1, f = 0; f < this.options.fields.length; f++)
								this.options.fields[f].match(/^(OBJECTID|FID|OID|ID)$/i) && (e = !0);
							e === !1 && console && console.warn && console.warn("no known esriFieldTypeOID field detected in fields Array.  Please add an attribute field containing unique IDs to ensure the layer can be drawn correctly.")
						}
						...
						this.options.timeField.start && this.options.timeField.end ? (this._startTimeIndex = new b, this._endTimeIndex = new b) : this.options.timeField && (this._timeIndex = new b),
					...
					_buildQuery : function (a) {
						...
						return this.options.simplifyFactor && b.simplify(this._map, this.options.simplifyFactor),
						"server" === this.options.timeFilterMode && this.options.from && this.options.to && b.between(this.options.from, this.options.to),
						b
					},
					setWhere : function (a, b, c) {
						this.options.where = a && a.length ? a : "1=1";
						for (var d = [], e = [], f = 0, g = null, h = L.Util.bind(function (a, h) {
									if (a && (g = a), h)
										for (var i = h.features.length - 1; i >= 0; i--)
											e.push(h.features[i].id);
										f--,
										0 >= f && (this._currentSnapshot = e, this.removeLayers(d), this.addLayers(e), b && b.call(c, g))
									}, this), i = this._currentSnapshot.length - 1; i >= 0; i--)d.push(this._currentSnapshot[i]);
						for (var j in this._activeCells) {
							f++;
							var k = this._keyToCellCoords(j),
							l = this._cellCoordsToBounds(k);
							this._requestFeatures(l, j, h)
						}
						return this
					},
					getWhere : function () {
					...
					setTimeRange : function (a, b, c, d) {
						...
						i = L.Util.bind(function (i) {
								i && (h = i),
								this._filterExistingFeatures(e, f, a, b),
								g--,
								c && 0 >= g && c.call(d, h)
							}, this);
					...
					_filterExistingFeatures : function (a, b, c, d) {
						var e = a && b ? this._getFeaturesInTimeRange(a, b) : this._currentSnapshot,
					...
					_buildTimeIndexes : function (a) {
						...
						if (this.options.timeField.start && this.options.timeField.end) {
							...
							this._startTimeIndex.bulkAdd(d),
							this._endTimeIndex.bulkAdd(e)
						} else {
							var f = [];
							for (b = a.length - 1; b >= 0; b--)
								c = a[b], f.push({
					...
					addFeature : function (a, b, c) {
						return this._service.addFeature(a, function (a, d) {
							a || this.refresh(),
							b && b.call(c, a, d)
						}, this),
						this
					...
					updateFeature : function (a, b, c) {
						return this._service.updateFeature(a, function (a, d) {
							a || this.refresh(),
							b && b.call(c, a, d)
						}, this)
					},
					deleteFeature : function (a, b, c) {
						return this._service.deleteFeature(a, function (a, d) {
							!a && d.objectId && this.removeLayers([d.objectId], !0),
							b && b.call(c, a, d)
						}, this)
					}
				}),
			b.prototype._query = function (a) {
				for (var b, c, d, e = 0, f = this.values.length - 1; f >= e; )
					if (d = b = (e + f) / 2 | 0, c = this.values[Math.round(b)], +c.value < +a)
						e = b + 1;
					else {
						if (!(+c.value > +a))
							return b;
		...
		EsriLeaflet.Layers.FeatureLayer = EsriLeaflet.Layers.FeatureManager.extend({
				...
				initialize : function (a, b) {
					EsriLeaflet.Layers.FeatureManager.prototype.initialize.call(this, a, b),
					b = L.setOptions(this, b),
					this._layers = {},
					this._leafletIds = {},
					this._key = "c" + (1e9 * Math.random()).toString(36).replace(".", "_")
				},
				onAdd : function (a) {
					return EsriLeaflet.Layers.FeatureManager.prototype.onAdd.call(this, a)
				},
				onRemove : function (a) {
					for (var b in this._layers)
						a.removeLayer(this._layers[b]);
				...
				removeLayers : function (a, b) {
					for (var c = a.length - 1; c >= 0; c--) {
						...
						e && b && delete this._layers[d]
					}
				},
				resetStyle : function (a) {
					var b = this._layers[a];
					return b && (b.options = b.defaultOptions, this.setFeatureStyle(b.feature.id, this.options.style)),
					this
				},
				setStyle : function (a) {
					return this.options.style = a,
				...
				setFeatureStyle : function (a, b) {
					...
					if ("function" == typeof b)
						b = b(c.feature);
					else if (!b && !c.defaultOptions) { {
							new L.Path
						}
						b = L.Path.prototype.options
					}
		...
		EsriLeaflet.Layers.featureLayer = function (a, b) {
			return new EsriLeaflet.Layers.FeatureLayer(a, b)
		},
		EsriLeaflet.featureLayer = function (a, b) {
			return new EsriLeaflet.Layers.FeatureLayer(a, b)
		},
		EsriLeaflet.Controls.Logo = L.Control.extend({
				...
				onAdd : function () {
					...
					return a.style.marginTop = this.options.marginTop,
					a.style.marginLeft = this.options.marginLeft,
					a.style.marginBottom = this.options.marginBottom,
					a.style.marginRight = this.options.marginRight,
					a.innerHTML = '<a href="https://developers.arcgis.com" style="border: none;"><img src="https://js.arcgis.com/3.10/js/esri/images/map/logo-med.png" style="border: none;"></a>',
					a
				}
			}),
